...
if 'initialized' not in st.session_state:
    st.session_state.memes = {
        ...
        'Dogecoin': {'price': 0.10, 'volatility': 0.3, 'mentions': 0},
...
def update_prices_with_real_data():
    ...
    for meme, data in st.session_state.memes.items():
        ...
        if meme == 'Dogecoin' and real_doge_price:
            ...
            st.session_state.memes[meme]['price'] = round(new_price, 4)
...
st.markdown("**Learn trading risk-free through memes with LIVE Dogecoin data**")
...
with st.sidebar:
    ...
    st.session_state.users += random.randint(0, 2)
    if random.random() < 0.05:
        st.session_state.revenue += 5
    
    st.metric("👥 Total Users", st.session_state.users)
    st.metric("💰 Monthly Revenue", f"${st.session_state.revenue}")
    ...
    # Monetization Section - WITH REAL PAYPAL LINKS
    st.header("💎 Meme Economist Pro")
    st.markdown("**Premium Features:**")
    st.markdown("✅ Real Reddit API data  \n✅ Live Twitter trends  \n✅ Portfolio alerts  \n✅ Advanced charts  \n✅ Priority support")

    # Payment options - REAL PAYPAL INTEGRATION
    col1, col2 = st.columns(2)
    
    with col1:
        if st.button("🚀 Monthly: $5", use_container_width=True, key="monthly_btn"):
            st.session_state.premium_clicks += 1
            st.markdown("[**👉 PayPal Subscription**](https://www.paypal.com/webapps/billing/plans/subscribe?plan_id=P-6E1926989Y1846907NDSAMQQ)")
            st.success("Redirecting to PayPal...")
    
    with col2:
        if st.button("💎 Lifetime: $49", use_container_width=True, key="lifetime_btn"):
            st.session_state.premium_clicks += 1
            st.info("Lifetime access coming soon! Email: your-email@gmail.com")
            st.success("We'll contact you for founding member access!")
...
st.markdown("---")
st.header("🎓 Learn Trading Risk-Free")
st.markdown("""
**The Meme Economist helps you understand market psychology through familiar concepts:**
- Practice with $10,000 virtual money
- See how social sentiment drives prices
- Learn portfolio management without risk
- Understand volatility through meme stocks
- Watch real Dogecoin price movements
""")
